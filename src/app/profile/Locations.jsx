...
export const Locations = ({ activeMenu, profileData }) => {
  ...
  return (
    <section
      className={`w-full flex-col gap-5 rounded-xl bg-white dark:bg-darkColor dark:text-white shadow-xl p-4 mt-4 ${
        activeMenu === "3" ? "flex" : "hidden"
      }`}
    >
      <article className="text-blue-800 dark:text-white">
        <span className="text-blue-800 font-bold flex gap-2 mb-2 dark:text-white">
          <Building className="w-6 h-6 text-blue-500 dark:text-white" />
          Ubicaciones
        </span>
      ...
      <article className="text-blue-800 dark:text-white">
        <span className="text-blue-800 font-bold flex gap-2 mb-2 dark:text-white">
          <ImageIcon className="w-6 h-6 text-blue-500 dark:text-white" />
          Fotos del consultorio
        </span>

        <GalleryLocation
          galleryID="my-test-gallery"
          images={profileData?.fotos}
        />
      </article>
    </section>
  );
