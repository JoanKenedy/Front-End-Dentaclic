...
export const EditProfile = () => {
  ...
  const changeImage = () => {
    ...
    fetch(endpoint, {
      ...
      body: JSON.stringify({ img: 'https://cdn.dentarios.com.mx/assets/images/imagen_2024-08-13_145525798.png' })
    })
      .then(async (response) => {
        if (!response.ok) {
        ...
        <div className='w-full flex justify-center items-center px-4 max-w-6xl'>
          {
            loading
              ? <LoadingData />
              : (
                <form className='w-full grid gap-4 max-w-xl' onSubmit={handleSubmit}>
                  <div className='w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max lg:row-span-3'>
                    ...
                    <div className='mt-4'>
                      <span className='text-[#151717] font-semibold'>Nombre Completo <span className='text-red-500'>*</span> </span>
                    </div>
                    ...
                    <div className='mt-4'>
                      <span className='text-[#151717] font-semibold'>Número de celular <span className='text-red-500'>*</span> </span>
                    </div>
                    ...
                    <div className='rounded-lg h-[38px] flex items-center w-full transition relative border-slate-200 border focus-within:border focus-within:border-blue-400'>
                      <input disabled id={mailId} type='text' value={personalData?.correo} className='bg-slate-100 pl-4 rounded-lg border-none w-full h-full focus:outline-none' />
                    </div>
                    <div className='mt-4'>
                      <span className='text-[#151717] font-semibold'>Contraseña</span>
                    </div>
                    <Link href='' className='w-max px-2 py-[2px] mt-2 rounded-lg bg-primaryBlue text-sm text-white text-center transition hover:bg-blue-950'>
                      Cambiar contraseña
                    </Link>
                    <button disabled={isLoading} type='submit' className='w-max px-4 mt-10 py-2 rounded-lg bg-primaryBlue text-white text-center transition hover:bg-blue-950'>
                      {isLoading ? <div aria-label='Cargando información' className='inline-block h-5 w-5 mr-2 animate-spin rounded-full border-2 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]' /> : 'Guardar'}
                    </button>
                  </div>
                </form>
                )
