...
export const FormEditProfile = () => {
  ...
  return loading ? (
    ...
    <form
      className="w-full grid gap-4 max-w-xl lg:max-w-full lg:grid-cols-2"
    ...
    >
      <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max lg:row-span-3">
        ...
        />
        <div className="mt-4">
          <label className="text-[#151717] font-semibold" htmlFor={nameInputId}>
        ...
        <div className="mt-4">
          <label className="text-[#151717] font-semibold" htmlFor={phoneId}>
      ...
      <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max">
        <span className="text-[#151717] font-semibold text-lg">
          Número de cédula <span className="text-red-500">*</span>
        </span>
        <span className="text-xs text-gray-500">
          Los cambios en el número de cédula deben ser aprobados por un
          moderador
        </span>
        ...
        <div>
          {cedulas?.map((cedula, index) => (
            <div key={index}>
              <div className="w-full flex gap-2 items-center justify-center h-[38px] mt-2">
                <div
                  className={`rounded-lg h-[38px] flex items-center w-full transition relative ${
                    cedulaError ? "border-red-500" : "border-slate-200"
                  } border focus-within:border focus-within:border-blue-400`}
                >
                  <input
                    disabled={isLoading}
                    ...
                    name={`cedula-${index}`}
                    value={cedula}
                    onChange={(e) => handleFieldChange("cedula", index, e)}
                    className="bg-slate-100 pl-4 rounded-lg border-none w-full h-full focus:outline-none"
                ...
                <button
                  disabled={isLoading}
                  type="button"
                  onClick={() => handleRemoveField("cedula", index)}
                >
          ...
          <button
            disabled={isLoading}
            type="button"
            className="px-2 py-[2px] mt-2 rounded-lg bg-primaryBlue text-white text-center transition hover:bg-blue-950"
            onClick={() => handleAddNewField("cedula")}
          >
            Agregar nueva
          </button>
        </div>
      ...
      <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max">
        <div className="grid">
          <span className="text-[#151717] font-semibold text-lg">
            Selecciona tu especialidad <span className="text-red-500">*</span>
          </span>
          <span className="text-red-500 text-[10px]">{specialtyError}</span>
          {specialties?.map((specialty, index) => (
            <div
              key={index}
              className="w-full flex gap-2 justify-center items-center mt-2"
            >
              <SelectSpecialty
                specialtyError={specialtyError}
                value={specialty}
                handleSelectSpecialty={handleFieldChange}
                idSpecialty={index}
              />
              <button
                disabled={isLoading}
                type="button"
                onClick={() => handleRemoveField("specialty", index)}
              >
                <Close className="w-6 text-red-500 transition rounded-full hover:bg-red-500 hover:text-white" />
              </button>
            </div>
          ...
          <button
            ...
            onClick={() => handleAddNewField("specialty")}
          >
            Agregar nueva
          </button>
        </div>
      ...
      <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max">
        ...
        <span className="text-xs text-gray-500">
          Para muchas personas, la educación médica es tan crucial como la
          experiencia profesional. Asegúrese de incluir detalles sobre su
        ...
        <div>
              ...
              <div className="w-full flex gap-2 items-center justify-center h-[38px] mt-2">
                <div className="rounded-lg h-[38px] flex items-center w-full transition relative border-slate-200 border focus-within:border focus-within:border-blue-400">
                  <input
                    ...
                    value={experience}
                    onChange={(e) => handleFieldChange("experiences", index, e)}
                    className="bg-slate-100 pl-4 rounded-lg border-none w-full h-full focus:outline-none"
          ...
          <button
            ...
            type="button"
            className="px-2 py-[2px] mt-2 rounded-lg bg-primaryBlue text-white text-center transition hover:bg-blue-950"
            onClick={() => handleAddNewField("experiences")}
          >
            Agregar nueva
          </button>
        </div>
      ...
      {!isRegister && (
        <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max lg:col-span-2">
          <span className="text-[#151717] font-semibold text-lg">
            Ingresa tu dirección <span className="text-red-500">*</span>
          </span>
          <span className="text-xs text-gray-500 mb-4">
            Ingresa la dirección de tu consultorio para que tus pacientes puedan
            encontrarte, puedes arrastrar el pin hasta el punto deseado.
          </span>
      ...
      <div className="w-full mt-4">
        <label className="text-[#151717] font-semibold" htmlFor="images">
          Subir imágenes
        </label>
        <input
          type="file"
          ...
          className="block w-full mt-2 p-2 border border-slate-200 rounded-md bg-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-400"
        />
        {imagesError && (
      ...
      <div className="w-full p-4 grid mt-3 gap-2 border border-slate-100 rounded-xl shadow-xl lg:h-max lg:row-span-2">
        ...
        <span className="text-xs text-gray-500">
          En esta sección, por favor, proporciona una breve descripción sobre
          ti.
        </span>
        <textarea
          name="aboutMe"
      ...
      <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max">
        ...
        <div>
              ...
              <div className="w-full flex gap-2 items-center justify-center h-[38px] mt-2">
                <div className="rounded-lg h-[38px] flex items-center w-full transition relative border-slate-200 border focus-within:border focus-within:border-blue-400">
                  <input
                    ...
                    name={`experiencia-${index}`}
                    value={education}
                    onChange={(e) => handleFieldChange("educations", index, e)}
                    className="bg-slate-100 pl-4 rounded-lg border-none w-full h-full focus:outline-none"
                    placeholder="IPN"
                  />
                </div>
                <button
                  disabled={isLoading}
                  type="button"
                  onClick={() => handleRemoveField("educations", index)}
                >
                  <Close className="w-6 text-red-500 transition rounded-full hover:bg-red-500 hover:text-white" />
                </button>
              </div>
          ...
          <button
            ...
            type="button"
            className="px-2 py-[2px] mt-2 rounded-lg bg-primaryBlue text-white text-center transition hover:bg-blue-950"
            onClick={() => handleAddNewField("educations")}
          >
            Agregar nueva
          </button>
        </div>
      ...
      <div className="w-full p-4 grid mt-3 border border-slate-100 rounded-xl shadow-xl lg:h-max">
        <div className="grid">
          ...
          <span className="text-xs text-gray-500">
            Incluye todos los idiomas que hablas. De esta manera, los visitantes
            extranjeros en tu ciudad también podrán ponerse en contacto contigo.
          </span>
          ...
          {languages?.map((language, index) => (
            ...
            >
              <SelectLanguage
                value={language}
                handleSelectLanguage={handleFieldChange}
                idLanguage={index}
              />
              <button
                disabled={isLoading}
      ...
      <div className="w-full p-4 flex flex-col justify-between items-center mt-3 border border-slate-100 rounded-xl shadow-xl md:flex-row lg:col-span-2">
        <span className="text-[#151717] font-semibold text-lg">
          Completa todo tu perfil para tener mejores resultados.
        </span>
        <button
          disabled={isLoading}
          type="submit"
          className="w-max px-4 py-2 mt-2 rounded-lg bg-primaryBlue text-white text-center transition hover:bg-blue-950"
        >
          {isLoading ? (
            <div
