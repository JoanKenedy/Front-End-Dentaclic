...
import { languagesArray } from '@/mocks/languages'
import { useRef, useState } from 'react'

export const SelectLanguage = ({ value, handleSelectLanguage, idLanguage }) => {
  ...
  const handleSelected = (event) => {
    setShowSelect(false)
    handleSelectLanguage('language', idLanguage, event)
  }

  return (
    <>
      <div className='relative w-full' ref={selectElement}>
        <button type='button' onClick={() => setShowSelect(!showSelect)} className='relative w-full cursor-default rounded-md bg-white py-1.5 pl-2 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 sm:text-sm sm:leading-6'>
