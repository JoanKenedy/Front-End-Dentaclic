...
export const Hero = () => {
  ...
  return (
    <section className="w-full justify-center items-center flex px-4">
      <div className="w-full flex justify-start items-center max-w-6xl py-4 bg-primaryBlue rounded-xl gap-8 shadow-lg relative">
        <img
          className="rounded-full aspect-square object-cover border-2 bg-white border-white w-1/6 max-w-56 shadow-lg ml-2"
        ...
        <span className="text-white">
          <h1 className="font-semibold flex gap-2 flex-wrap items-center md:text-2xl">
            {personalData?.nombre}{" "}
            <span>
              <Check className="w-4 h-4 p-[2px] bg-blue-500 rounded-full text-white" />
            </span>
          </h1>
          <p className="text-xs md:text-base">{personalData?.correo}</p>
          {userData.especialidades && (
            <span className="bg-blue-500 px-2 rounded-full text-xs md:text-base">
              {userData?.especialidades[0]}
            </span>
          )}
        </span>
